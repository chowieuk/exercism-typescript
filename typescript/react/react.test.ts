...
describe('React module', () => {
  ...
  it.ignore('input cells have a value', () => {
  ...
  it.ignore("an input cell's value can be set", () => {
    const newValue = 20
  ...
  it.ignore('compute cells calculate initial value', () => {
  ...
  it.ignore('compute cell takes inputs in correct order', () => {
  ...
  it.ignore('compute cells can depend on other compute cells', () => {
    const [input, setInput] = createInput(1)
  ...
  it.ignore('compute cells fire callbacks', () => {
    const [input, setInput] = createInput(1)
    const output = createComputed(() => input() + 1)
    let value = 0
  ...
  it.ignore('callbacks fire only when output values change', () => {
    const [input, setInput] = createInput(1)
  ...
  it.ignore('callbacks do not report already reported values', () => {
    const [input, setInput] = createInput(1)
    const output = createComputed(() => input() + 1)
  ...
  it.ignore('callbacks can fire from multiple cells', () => {
    const [input, setInput] = createInput(1)
  ...
  it.ignore('static callbacks fire even if their own value has not changed', () => {
    const [input, setInput] = createInput(1)
    const output = createComputed(
      () => (input() < 3 ? 111 : 222),
      undefined,
  ...
  it.ignore('callbacks can be added and removed', () => {
  ...
  it.ignore("removing a callback multiple times doesn't interfere with other callbacks", () => {
    const [input, setInput] = createInput(1)
    const output = createComputed(() => input() + 1)
  ...
  it.ignore('callbacks should only be called once, even if multiple dependencies change', () => {
    const [input, setInput] = createInput(1)
  ...
  it.ignore("callbacks should not be called if dependencies change but output value doesn't change", () => {
    const [input, setInput] = createInput(1)
    const plusOne = createComputed(() => input() + 1)
