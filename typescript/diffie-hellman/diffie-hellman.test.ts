...
describe('diffie-hellman', () => {
  ...
  it.ignore('throws an error if the constructor arguments are not prime', () => {
    expect(() => {
  ...
  describe('input validation', () => {
    ...
    it.ignore('throws an error if private key is negative', () => {
      expect(() => {
        diffieHellman.getPublicKey(-1)
      }).toThrow()
    })

    it.ignore('throws an error if private key is zero', () => {
      expect(() => {
        diffieHellman.getPublicKey(0)
      }).toThrow()
    })

    it.ignore('throws an error if private key is one', () => {
      expect(() => {
        diffieHellman.getPublicKey(1)
      }).toThrow()
    })

    it.ignore('throws an error if private key equals the modulus parameter p', () => {
      expect(() => {
        diffieHellman.getPublicKey(p)
      }).toThrow()
    })

    it.ignore('throws an error if private key is greater than the modulus parameter p', () => {
      expect(() => {
  ...
  describe('stateless calculation', () => {
    ...
    it.ignore('can calculate public key using private key', () => {
      expect(diffieHellman.getPublicKey(alicePrivateKey)).toEqual(
        alicePublicKey
      )
    })

    it.ignore('can calculate public key when given a different private key', () => {
      expect(diffieHellman.getPublicKey(bobPrivateKey)).toEqual(bobPublicKey)
    })
  })

  it.ignore("can calculate secret using other party's public key", () => {
    expect(new DiffieHellman(23, 5).getSecret(19, 6)).toEqual(2)
  })

  it.ignore('key exchange', () => {
    const diffieHellman = new DiffieHellman(23, 5)
