...
describe('Queens', () => {
  describe('Test creation of Queens with valid and invalid positions', () => {
    ...
    it.ignore('queen must have positive row', () => {
    ...
    it.ignore('queen must have row on board', () => {
      const positioning = { white: [8, 4] } as const
      const expectedError = 'Queen must be placed on the board'
      expect(() => new QueenAttack(positioning)).toThrow(expectedError)
    })

    it.ignore('queen must have positive column', () => {
      const positioning = { white: [2, -2] } as const
      const expectedError = 'Queen must be placed on the board'
      expect(() => new QueenAttack(positioning)).toThrow(expectedError)
    })

    it.ignore('queen must have column on board', () => {
      const positioning = { white: [4, 8] } as const
      const expectedError = 'Queen must be placed on the board'
      expect(() => new QueenAttack(positioning)).toThrow(expectedError)
    })

    it.ignore('two queens cannot occupy the same space', () => {
  ...
  describe('Test the ability of one queen to attack another', () => {
    it.ignore('queens cannot attack', () => {
      const queens = new QueenAttack({ white: [2, 4], black: [6, 6] })
      expect(queens.canAttack).toEqual(false)
    })

    it.ignore('queens can attack when they are on the same row', () => {
      const queens = new QueenAttack({ white: [2, 4], black: [2, 6] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack when they are on the same column', () => {
      const queens = new QueenAttack({ white: [4, 5], black: [2, 5] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack diagonally', () => {
      const queens = new QueenAttack({ white: [2, 2], black: [0, 4] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack another diagonally', () => {
      const queens = new QueenAttack({ white: [2, 2], black: [3, 1] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack yet another diagonally', () => {
      const queens = new QueenAttack({ white: [2, 2], black: [1, 1] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack diagonally, really', () => {
      const queens = new QueenAttack({ white: [1, 7], black: [0, 6] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack on a north-east/south-west diagonal', () => {
      const queens = new QueenAttack({ white: [7, 0], black: [0, 7] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens can attack on another ne/sw diagonal', () => {
      const queens = new QueenAttack({ white: [2, 6], black: [5, 3] })
      expect(queens.canAttack).toEqual(true)
    })

    it.ignore('queens cannot attack if falling diagonals are only the same when reflected across the longest falling diagonal', () => {
  ...
  describe('Test the board visualisation', () => {
    ...
    it.ignore('board with queens at their starting positions', () => {
    ...
    it.ignore('board with the black queen at her starting positions', () => {
    ...
    it.ignore('board with queens at the edges', () => {
